 ...
 * ============================================
 * 
 * Comprehensive shopping cart functionality with advanced features:
 * - Debounced quantity updates (300ms delay)
 * - Optimistic UI updates with rollback capability
 * - Race condition prevention with version control
...
// Cart management namespace to prevent global scope pollution
window.CartManager = (function() {
     ...
     * Sets up event listeners for all cart operations
     * 
     * @public
     */
    function init() {
    ...
    function updateQuantityOnServer(itemId, newQuantity, adjuster, originalQuantity) {
        // Get CSRF token from the adjuster or from a global token
     ...
     * Restores original values and shows error message
     * 
     * @private
     * @param {HTMLElement} adjuster - Quantity adjuster element
     ...
     * Disables buttons and shows loading spinners
     * 
     * @private
     * @param {HTMLElement} adjuster - Quantity adjuster element
     ...
     * Updates subtotal, total, discount, and item count
     * 
     * @private
    ...
    function updateCartTotals(totals) {
        ...
        if (totals.total !== undefined) {
            ...
            // Also update navbar cart total
            updateCartTotalInNavbar(totals.total);
        }
        ...
        if (totals.item_count !== undefined) {
            updateCartCountBadge(totals.item_count);
        }
    }
    ...
    /**
     * Update cart total in navbar sticky cart preview
     * 
     * @private
     * @param {number} total - New cart total amount
     */
    function updateCartTotalInNavbar(total) {
        // Update sticky cart preview total
        ...
        // Update the main navbar cart total if it exists
        const navbarTotal = document.querySelector('#navbar-cart-total');
        if (navbarTotal) {
            navbarTotal.textContent = formatCurrency(total);
        }
    }
    ...
    function removeItemFromCart(itemId, button) {
        ...
        .then(data => {
            if (data.success) {
                ...
                // Animate removal
                cartItem.style.transition = `all ${ANIMATION_DURATION}ms ease-out`;
    ...
    function showPromoSuccess(promoCode) {
        ...
        if (nameSpan) nameSpan.textContent = promoCode.code;
    ...
    function showNotification(message, type = 'info') {
        ...
        notification.className = `p-4 rounded-lg shadow-lg fade-in max-w-sm transform transition-all duration-300 scale-95 opacity-0 ${getNotificationClass(type)}`;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (document.querySelector('.cart-container') || document.querySelector('.quantity-adjuster')) {
        CartManager.init();
    }
});
